...
import { resetDatabase } from './seed_better/reset'
import { seedVipLevels } from './seed_better/vip'
import { seedGames } from './seed_better/games'
import { seedOperator } from './seed_better/operator'
import { seedProducts } from './seed_better/products'
import { seedUsers, seedHardcodedUser, seedWallets } from './seed_better/users'
import { seedGameSpins } from './seed_better/gameSpins' // 1. Import the new seeder
...
async function main() {
  ...
  try {
    ...
    await seedWallets(db as any, operator.id)
  } catch (error) {
