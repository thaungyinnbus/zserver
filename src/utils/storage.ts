...
class StorageManager {
  ...
  isStale(): boolean {
    const times: number[] = []
    for (let i = 0; i < this.storage.length; i++) {
      const key: string = this.storage.key(i) as string
      const itemStr = this.storage.getItem(key) as string
      const item: StorageItem<any> = destr(itemStr);
      if (item != null && item.expiry) {
        times.push(item.expiry)
        // console.log(item.expiry)
      }
    }
    const rawtime = times.sort((a, b) => b - a)[0]
    console.log('rawtime ', rawtime)
    const now = new Date().getTime()
    console.log('now ', now)
    const diff = rawtime - now
    console.log(diff)
    const sec = diff / 60 / 1000
    console.log(Math.floor(sec))
    console.log(Math.floor(sec) / 60)
    return (sec > 0) ? false : true
  }
  /**
   * 获取存储项
   * @param key 键
